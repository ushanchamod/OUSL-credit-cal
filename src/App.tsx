...
import UnknownCoursesNotification from "./components/UnknownCoursesNotification";
...
const Home = () => {
  ...
  useEffect(() => {
    if (data) {
      const unknown = data
        .filter((course) => !getCourse(course.code))
        .map((course) => course.code);
      setUnknownCourses([...new Set(unknown)]);
    }
  }, [data, getCourse]);
      ...
      <main className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        {hasData && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <UnknownCoursesNotification
              unknownCourses={unknownCourses}
              onCourseAdded={() => {
                // Refresh unknown courses list
                const unknown = data
                  .filter((course) => !getCourse(course.code))
                  .map((course) => course.code);
                setUnknownCourses([...new Set(unknown)]);
